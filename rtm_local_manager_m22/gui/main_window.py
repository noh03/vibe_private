...
class IssueTabWidget(QTabWidget):
    ...
    def set_issue(self, issue: Dict[str, Any] | None) -> None:
        ...
        # Attachments 리스트 표시 (attachments 컬럼은 JIRA JSON 배열을 담은 문자열)
        if hasattr(self, "attachments_list"):
...
class MainWindow(QMainWindow):
    def __init__(self, db_path=None, config_path: str = "jira_config.json", mode: str = "both"):
        ...
        # Local (SQLite only)
        # ------------------------------------------------------------------
        grp_local = QGroupBox("Local (SQLite only)")
    ...
    def on_refresh_activity_clicked(self):
        ...
        try:
            ...
            if isinstance(comments, list):
                # IssueTabWidget 에 원본 댓글 목록 캐시
                self.left_panel.issue_tabs.set_activity_comments(comments)
                ...
                # 댓글이 없으면 캐시도 비운다.
                self.left_panel.issue_tabs.set_activity_comments([])
    ...
    def on_add_comment_clicked(self):
        ...
        if self.current_issue_id is None:
            self.status_bar.showMessage("No issue selected; cannot add comment.")
            return

        issue = get_issue_by_id(self.conn, self.current_issue_id)
        if not issue:
            self.status_bar.showMessage("Issue not found in local DB; cannot add comment.")
            return

        jira_key = issue.get("jira_key")
        if not jira_key:
            self.status_bar.showMessage("Selected issue has no JIRA Key; cannot add comment.")
            return
    ...
    def _get_current_issue_jira_key(self) -> tuple[str | None, Dict[str, Any] | None]:
        """
        현재 선택된 이슈의 jira_key 와 로컬 issue dict 를 반환하는 헬퍼.
        JIRA 연동 관련 슬롯들에서 공통 사용.
        """
        if self.current_issue_id is None:
            return None, None
        issue = get_issue_by_id(self.conn, self.current_issue_id)
    ...
    def on_edit_comment_clicked(self):
        ...
        jira_key, issue = self._get_current_issue_jira_key()
        if not jira_key or issue is None:
            self.status_bar.showMessage("Selected issue has no JIRA Key; cannot edit comment.")
            return

        comments = self.left_panel.issue_tabs.get_activity_comments()
        if not comments:
    ...
    def on_delete_comment_clicked(self):
        ...
        jira_key, issue = self._get_current_issue_jira_key()
        if not jira_key or issue is None:
            self.status_bar.showMessage("Selected issue has no JIRA Key; cannot delete comment.")
            return
    ...
    def on_upload_attachment_clicked(self):
        """
        현재 이슈에 대해 JIRA 첨부를 업로드한다.
        - 파일 선택 다이얼로그에서 파일을 고른 뒤 add_issue_attachment_from_path 호출
        - 성공 시 JIRA Issue 를 다시 조회하여 attachments 필드를 동기화하고 UI 갱신
        """
        if not self.jira_available or not self.jira_client:
            self.status_bar.showMessage("JIRA is offline; cannot upload attachment.")
            return

        if self.current_issue_id is None:
            self.status_bar.showMessage("No issue selected; cannot upload attachment.")
            return

        issue = get_issue_by_id(self.conn, self.current_issue_id)
        if not issue:
            self.status_bar.showMessage("Issue not found in local DB; cannot upload attachment.")
            return

        jira_key = issue.get("jira_key")
        if not jira_key:
            self.status_bar.showMessage("Selected issue has no JIRA Key; cannot upload attachment.")
            return

        file_path, _ = QFileDialog.getOpenFileName(
            self,
            f"Upload Attachment to {jira_key}",
            "",
            "All Files (*)",
        )
        if not file_path:
        ...
        try:
            self.status_bar.showMessage(f"Uploading attachment to {jira_key}...")
            QApplication.setOverrideCursor(Qt.WaitCursor)
            self.jira_client.add_issue_attachment_from_path(jira_key, file_path)
            # 업로드 후 JIRA 이슈를 다시 로딩하여 attachments 동기화
            issue_type = issue.get("issue_type") or ""
            self._reload_issue_from_jira_standard(issue_type, jira_key)
            self.status_bar.showMessage(f"Uploaded attachment to {jira_key}")
        except Exception as e:
            self.status_bar.showMessage(f"Failed to upload attachment: {e}")
            print(f"[ERROR] Failed to upload attachment to {jira_key}: {e}")
        finally:
            QApplication.restoreOverrideCursor()

    def on_delete_attachment_clicked(self):
        """
        Details 탭의 첨부 리스트에서 선택된 항목을 JIRA 에서 삭제한다.
        - 선택된 아이템에서 attachment id 를 읽어 delete_issue_attachment 호출
        - 성공 시 JIRA Issue 를 다시 조회하여 attachments 필드를 동기화하고 UI 갱신
        """
        if not self.jira_available or not self.jira_client:
            self.status_bar.showMessage("JIRA is offline; cannot delete attachment.")
            return

        if self.current_issue_id is None:
            self.status_bar.showMessage("No issue selected; cannot delete attachment.")
            return
        ...
        att_id = current_item.data(Qt.UserRole)
        if not att_id:
            self.status_bar.showMessage("Selected attachment has no id; cannot delete.")
            return

        issue = get_issue_by_id(self.conn, self.current_issue_id)
        if not issue:
            self.status_bar.showMessage("Issue not found in local DB; cannot delete attachment.")
            return

        jira_key = issue.get("jira_key")
        if not jira_key:
            self.status_bar.showMessage("Selected issue has no JIRA Key; cannot delete attachment.")
            return

        confirm = QMessageBox.question(
            self,
            "Delete Attachment",
            f"Delete selected attachment from {jira_key}?",
            QMessageBox.Yes | QMessageBox.No,
            QMessageBox.No,
        )
        if confirm != QMessageBox.Yes:
        ...
        try:
            self.status_bar.showMessage(f"Deleting attachment from {jira_key}...")
            QApplication.setOverrideCursor(Qt.WaitCursor)
            self.jira_client.delete_issue_attachment(att_id)
            # 삭제 후 JIRA 이슈를 다시 로딩하여 attachments 동기화
            issue_type = issue.get("issue_type") or ""
            self._reload_issue_from_jira_standard(issue_type, jira_key)
            self.status_bar.showMessage(f"Deleted attachment from {jira_key}")
        except Exception as e:
            self.status_bar.showMessage(f"Failed to delete attachment: {e}")
            print(f"[ERROR] Failed to delete attachment from {jira_key}: {e}")
        finally:
            QApplication.restoreOverrideCursor()

    def on_open_attachment_clicked(self):
        """
        Details 탭의 첨부 리스트에서 선택된 항목을 브라우저(또는 OS 기본 핸들러)로 연다.
        - JIRA REST 가 제공하는 attachment content URL 을 그대로 open_url 로 연동
        """
        import webbrowser
        ...
        if not url:
            self.status_bar.showMessage("Selected attachment has no URL.")
            return
