...
class MainWindow(QMainWindow):
    ...
    def _connect_signals(self):
        ...
        if self.jira_available:
            ...
            if hasattr(self.right_panel, "btn_online_create"):
                ...
                self.logger.info("[SIGNAL] ✅ btn_online_create (트리 툴바) 연결 완료")
    ...
    def on_create_new_online_issue_clicked(self):
        ...
        REST API를 사용하여 이슈 타입별 POST 요청을 수행한다.
        """
        self.logger.info("[Create in JIRA/RTM] 버튼 클릭됨 (트리 툴바 또는 헤더 버튼)")
        
        if not self.jira_available or not self.jira_client:
            self.logger.warning("[Create in JIRA/RTM] JIRA RTM이 구성되지 않음")
            self.status_bar.showMessage("Cannot create: Jira RTM not configured.")
            return
        
        if not self.project or not self.project.project_key:
            self.logger.warning("[Create in JIRA/RTM] 프로젝트 키가 없음")
        ...
        self.logger.info(f"[Create in JIRA/RTM] 현재 모듈 탭 인덱스: {current_tab_index}, 이슈 타입: {issue_type}")
        
        if not issue_type:
            self.logger.warning("[Create in JIRA/RTM] 이슈 타입이 없음 (모듈 탭 선택 필요)")
                ...
                if selected_indexes:
                    ...
                    if item:
                        ...
                        if node_type == "FOLDER":
                            ...
                            self.logger.info(f"[Create in JIRA/RTM] 선택된 폴더의 parent_test_key: {parent_test_key}")
        
        self.logger.info(f"[Create in JIRA/RTM] CreateIssueDialog 표시 시작: issue_type={issue_type}, project_key={self.project.project_key}, parent_test_key={parent_test_key}")
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            self.logger.info(f"[Create in JIRA/RTM] 다이얼로그 Accepted, created_key: {created_key}")
            if created_key:
                self.status_bar.showMessage(f"Created {issue_type} in JIRA as {created_key}.")
                self.logger.info(f"[Create in JIRA/RTM] ✅ 이슈 생성 성공: {issue_type} - {created_key}")
                # 온라인 트리 새로고침
                try:
                    self.on_refresh_online_tree()
                    # 생성된 이슈를 트리에서 찾아서 선택
                    ...
                    self.logger.warning(f"Failed to refresh online tree after creation: {e}", exc_info=True)
            else:
                self.logger.warning("[Create in JIRA/RTM] 이슈 생성되었으나 키를 반환받지 못함")
                self.status_bar.showMessage("Issue created but key not returned.")
        else:
            self.logger.info("[Create in JIRA/RTM] 다이얼로그 취소됨")
